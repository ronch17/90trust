...
docker exec 90trust-db-1 sh -c 'exec mariadb-dump wordpress -uwordpress -p'${DB_PASSWORD}|gzip -9 > ~/${LOCAL_WORDPRESS_DIRECTORY}/${LOCAL_DIRECTORY}/backup/__after-commit__~${TRIM_LAST_COMMIT}~__wp-${LOCAL_DB_EXPORT_NAME}-`date +%F-%T`.sql.gz
if [ $? != 0 ];then
