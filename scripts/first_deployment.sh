...
ssh-keygen
cat ~/.ssh/id_rsa.pub
# add to https://bitbucket.org/binaricore/90trust/admin/access-keys/ as a new key
...
git clone git@bitbucket.org:binaricore/90trust
cd 90trust/ && git config core.filemode false && cd
mkdir ssl-for-origin/ && cd ssl-for-origin && sudo openssl req -x509 -nodes -days 3650 -newkey rsa:4096 -keyout 90trust.key -out 90trust.crt && cd


# In project directory (cd ~/wordpress/90trust) run deploy bash script sh ./scripts/deploy_dest.sh && sh ./scripts/restart_dest.sh
...
#Deploy 90trust.com.key & 90trust.com.crt for root user:
scp -r /home/${LOCAL_USER}/ssl-keys/90trust/ssl-for-origin  root@{DEST_SERVER_IP}:/home/proftit/
#OR for AMZ ec2-user
scp -i .ssh/90trust -r /home/${LOCAL_USER}/ssl-keys/90trust/ssl-for-origin ec2-user@{DEST_SERVER_IP}:~/
